...
| `proxy` | Рестарт `l2-proxy` под нагрузкой | In-flight запросы завершаются 5xx или обрывом соединения (никогда не виснут), после восстановления прокси отвечает 200; целостность сообщений не нарушена |
...
- **Спарклайны активности** — под значениями каждой плитки мини-Grafana график
  за последние N минут (по умолчанию 30, параметр `?window=N`, до 120). Данные
  берутся из **внутрипроцессного кольцевого буфера** (`MetricsHistory`):
  фоновый сэмплер раз в 15 с снимает значения registry, `/stats` лишь читает
  буфер — это дёшево и не зависит от числа открытий страницы (без обращений к
  VictoriaMetrics). Для counter и histogram/summary рисуется **скорость**
  (дельта/сек), для gauge — сырое значение. Представительная серия плитки —
  ненаклейменная (total) либо самая активная из размеченных.
...
Замер 2026-09-12 (после рефакторинга proxy-init/exposer, код hot path не менялся): 2000×100 — p50 205ms, p95 324ms, p99 414ms, 0 ошибок; 5000×200 — p99 ~1s, p999 ~10s. Вывод: хвост растёт только когда concurrency превышает пул воркера (`L2_WORKER_THREADS=128`, см. `l2_worker_queue_size`/`in_flight` метрики) — это queueing, не регрессия. Держать concurrency ≤ ~100 для p99 < 500ms либо крутить `L2_WORKER_THREADS`.
...
**Текущие счётчики** (после раундов 11b–13e):

| Тестовый бинарь | Test cases | Assertions | Фокус |
|---|---|---|---|
| `test_components` | 344 | 1 470 | Core-компоненты (см. ключевые модули ниже) |
