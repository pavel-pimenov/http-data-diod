...
public:
  ...
  explicit DedupCache(bool enabled = true, size_t max_entries = dedup_default_max(),
      ...
      : m_config{enabled, max_entries, ttl_ms} {}
  ...
  find(std::string_view request_id) const {
    ...
    const auto it = m_state.m_entries.find(std::string(request_id));
    if (it == m_state.m_entries.end()) {
      return std::nullopt;
    }
    if (it->second.m_expires_at_ms <= now_ms) {
      // m_state.m_entries / m_state.m_order are mutable, so erasing from a
      // const method is safe (still guarded by the mutex).
      m_state.m_order.erase(it->second.m_order_iter);
      m_state.m_entries.erase(it);
      return std::nullopt;
    }
  ...
  void store(std::string_view request_id, std::string response_json) {
    ...
    if (it != m_state.m_entries.end()) {
      ...
      it->second.m_expires_at_ms = now_ms + m_config.m_ttl_ms;
      // O(1): splice the existing node to the back of the list
      m_state.m_order.splice(m_state.m_order.end(), m_state.m_order,
                             it->second.m_order_iter);
      return;
    }

    if (m_config.m_max_entries != 0) {
      while (m_state.m_entries.size() >= m_config.m_max_entries) {
        evict_oldest_locked();
      }
    }
    m_state.m_order.push_back(key);
    auto order_iter = std::prev(m_state.m_order.end());
    m_state.m_entries.emplace(
        key, Entry{std::move(response_json), now_ms + m_config.m_ttl_ms,
                   order_iter});
  }
...
private:
  ...
  void evict_expired_locked(uint64_t now_ms) {
    while (!m_state.m_order.empty()) {
      const auto it = m_state.m_entries.find(m_state.m_order.front());
      if (it == m_state.m_entries.end() ||
          it->second.m_expires_at_ms > now_ms) {
        break;
      }
      m_state.m_entries.erase(it);
      m_state.m_order.pop_front();
    }
  }

  void evict_oldest_locked() {
    if (m_state.m_order.empty()) {
      return;
    }
    m_state.m_entries.erase(m_state.m_order.front());
    m_state.m_order.pop_front();
  }
};
