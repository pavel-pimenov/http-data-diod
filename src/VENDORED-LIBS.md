...
| [nats.c (cnats)](https://github.com/nats-io/nats.c) | 3.14.0-beta | source tree `nats/` (src/) | пин — коммит 9cae3733 (ветка main); проект использует только классическое publish/subscribe без JetStream; локальная правка `CMakeLists.txt`: examples/test отключены опциями `NATS_BUILD_EXAMPLES`/`BUILD_TESTING` |
| [cpp-httplib](https://github.com/yhirose/cpp-httplib) | 0.56.0 | `httplib/httplib.h` + `httplib.cc` | пин `v0.56.0`; пара заголовок+реализация собирается из single-header штатным `split.py` этого же тега; реализация не правится (сторонняя либа) |
