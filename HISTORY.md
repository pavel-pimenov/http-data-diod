...
- Результат: `re-sends delta=12` → `cache-hits delta=12` (каждый ресенд —
  хит), `l2_calls == processed` (19/19), exit=0. Детерминировано с 1-й
  попытки. Добавлены хелпер `compose_up(service, extra_env)` и константа
  `SERVER_METRICS_URL`; `ruff check` чисто.
