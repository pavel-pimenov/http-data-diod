...
- НО: в полном suite (после сценариев 1–3) — 0/2, другая сигнатура:
  ...
  пары 1+4/2+4/3+4 и тройка 1+2+4 — PASS; ломается только полный набор.
  Исключено: утечка delay (restore проверен `=0`), вина `compose_up`
  (микро-тест: `up -d l2-server` proxy не трогает), вина воркера
  (worker+dedup PASS), падение proxy (restarts=0). Открыто: источник
  сброса 15 клиентских TCP в полном прогоне; кандидатов в коде сценария
  и моём диффе не осталось — UNDER INVESTIGATION, не claim'лю 9/9.
